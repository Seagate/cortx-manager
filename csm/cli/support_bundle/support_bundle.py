...
from csm.common.errors import CSM_OPERATION_SUCCESSFUL, CsmError
...
class SupportBundle:
    ...
    async def fetch_host_from_salt():
        """
        This method fetches hostname for all nodes from Salt DB

        :return hostnames: List of Hostname :type: List
        :return node_list: List of Node Name :type: List
        """

        process = SimpleProcess("salt-call pillar.get cluster:node_list --out=json")
        ...
        for each_node in nodes:
            process = SimpleProcess(f"salt-call pillar.get cluster:{each_node}:hostname --out=json")
            _out, _err, _rc = process.run()
            if _rc != 0:
                Log.warn(f"Salt command failed : {_err}")
                return None, None
    ...
    async def fetch_host_from_cluster():
        ...
        if not cluster_file_path or not os.path.exists(cluster_file_path):
            response_msg = {"message": (
                f"{cluster_file_path} not Found. \n"
                "Please check if cluster info file is correctly configured.")}
            return Response(rc=errno.ENOENT, output=response_msg), None
    ...
    async def bundle_generate(command) -> sys.stdout:
        ...
        if current_user.lower() != const.SSH_USER_NAME:
            ...
            return Response(rc=errno.EACCES, output=response_msg)
        ...
        sos = bool(command.options.get(const.SOS_COMP, False) == "true")
        ...
        for index, hostname in enumerate(hostnames):
            ...
            thread_obj = Thread(SupportBundle.execute_ssh(hostname, current_user, ssh_key,
        ...
        return Response(output=response_msg, rc=errors.CSM_OPERATION_SUCCESSFUL)
    ...
    async def bundle_status(command):
        ...
        return Response(output=response, rc=errors.CSM_OPERATION_SUCCESSFUL)
    ...
    async def configure(command):
        ...
        csm_conf_file_name = os.path.join(const.CSM_CONF_PATH, const.CSM_CONF_FILE_NAME)
        if not os.path.exists(csm_conf_file_name):
            raise CsmError(errno.ENOENT, "Config file is not exist")
    ...
    async def show_config(command):
        ...
        return Response(output=support_bundle_config, rc=CSM_OPERATION_SUCCESSFUL)
