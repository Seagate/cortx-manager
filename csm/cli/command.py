...
class Validators:
    ...
    def file_parser(value):
        try:
            ...
            Log.error(f"File parsing failed. {value}: {e}")
            raise ArgumentError(
                errno.EINVAL, "File operations failed. Please check if the file is valid or not")
        except FileNotFoundError as e:
            Log.error(f"No such file present. {value}: {e}")
            raise ArgumentError(
                errno.ENOENT, "File operation failed. Please check if the file exists.")
...
class CommandParser:
    ...
    def handle_subparsers(self, sub_parser, data: Dict, name, add_parser_flag=True):
        ...
        :param data: Data for parsing the commands
        ...
        """
        if add_parser_flag:
            sub_parser = sub_parser.add_parser(data['name'], help=data['description'])
    ...
    def check_permissions(self, sub_command):
        """
        filter sub_command if found any permissions tag if no permissions tag is found
        it returns true
        """
...
class Output:
    ...
    def dump(self, out, err, output_type, **kwargs) -> None:
        ...
        if self.rc not in (200, 201, CSM_OPERATION_SUCCESSFUL):
            ...
            output = getattr(Output, f'dump_{output_type}')(self.output, **kwargs)
            out.write(f"{output}\n")
    ...
    def dump_table(data: Any, table: Dict, **kwargs: Dict) -> str:
        ...
        :param table: Table Output From Json
