...
class CortxCli(Cmd):
    ...
    def preloop(self):
        ...
        syslog_port = Conf.get(const.CORTXCLI_GLOBAL_INDEX, "Log>syslog_port")
        backup_count = Conf.get(const.CORTXCLI_GLOBAL_INDEX, "Log>total_files")
        file_size_in_mb = Conf.get(const.CORTXCLI_GLOBAL_INDEX, "Log>file_size")
        Log.init("cortxcli",
             syslog_server = Conf.get(const.CORTXCLI_GLOBAL_INDEX, "Log>syslog_server"),
             ...
             level = Conf.get(const.CORTXCLI_GLOBAL_INDEX, "Log>log_level"))
        if ( Conf.get(const.CORTXCLI_GLOBAL_INDEX, "DEPLOYMENT>mode") != const.DEV ):
            Security.decrypt_conf(const.CORTXCLI_GLOBAL_INDEX)
        #Set Rest API for CLI
        csm_agent_port = Conf.get(const.CORTXCLI_GLOBAL_INDEX,'CORTXCLI>csm_agent_port')
        csm_agent_host = Conf.get(const.CORTXCLI_GLOBAL_INDEX,'CORTXCLI>csm_agent_host')
        csm_agent_base_url = Conf.get(const.CORTXCLI_GLOBAL_INDEX,'CORTXCLI>csm_agent_base_url')
