...
from csm.common.payload import Dict, Json, JsonMessage, Payload
from csm.common.plugin import CsmPlugin
from csm.common.services import Service
from csm.core.blogic import const

try:
    from cortx.utils.ha.dm.decision_maker import DecisionMaker  # pylint: disable=ungrouped-imports
...
class AlertSchemaValidator(Schema):
    ...
    alert_uuid = fields.String(required=True, description="uuid to identify an  alert.")
    sensor_info = fields.String(
        required=True,
        description="unique key to identify hardware."
                    "Combination of site_id,node_id,rack_id,resource_id,cluster_id")
    state = fields.String(
        required=True, description="State of the alert (e.g. missing| insertion etc.)")
    created_time = fields.Integer(required=True, description="Origination time of the alert")
    updated_time = fields.Integer(required=False, description="Updation time of the alert")
    resolved = fields.Boolean(
        required=False, description="Resolution status of an alert. (e.g. True | False)")
    acknowledged = fields.Boolean(
        required=False, description="Alert is acknowledged by the user or not. (e.g. True | False)")
    severity = fields.String(
        required=True, description="Severity of an alert.(e.g. TBD")
    module_type = fields.String(
        required=False, description="Type of the module. (e.g. PSU, FAN)")
    module_name = fields.String(
        required=False, description="Name of the module. (e.g. Fan Module 4)")
    description = fields.String(
        required=False, allow_none=True,
        description="This is the id from which a description will be fetched (e.g. TBD)")
    health = fields.String(
        required=False, allow_none=True,
        description="Describes the health of PSU, Controller, Disk etc.")
    health_recommendation = fields.String(
        required=False, allow_none=True, description="This is the health recommendation string.")
    extended_info = fields.String(required=False, description="Extended Info")
    event_details = fields.String(required=False, description="Specific fields to display.")
    name = fields.String(required=False, allow_none=True, description="Name for specific modules.")
    serial_number = fields.String(
        required=False, allow_none=True, description="Serial number of the resource.")
    volume_group = fields.String(
        required=False, allow_none=True, description="Disk volume group.")
    volume_size = fields.String(required=False, allow_none=True, description="Disk size.")
    volume_total_size = fields.String(
        required=False, allow_none=True, description="Disk total size.")
    version = fields.String(
        required=False, allow_none=True, description="Version information for resources.")
    location = fields.String(
        required=False, allow_none=True, description="Location of the resources.")
    disk_slot = fields.Integer(
        required=False, allow_none=True, description="Slot number of the disk.")
...
class AlertPlugin(CsmPlugin):
    """
    Alert Plugin is responsible for listening on the comm channel and receive alerts.
    It has a callback which is called to send the received alerts.
    ...
    def init(self, callback_fn, health_plugin):
        ...
        :param callback_fn: This parameter specifies the name AlertMonitor
        ...
        try:
            ...
            Log.error(f"Error occurred while calling alert plugin init. {e}")
    ...
    def _plugin_callback(self, message):
        """
        1. This is the callback method on which we will receive the alerts from Comm class.
        2. This method will call AlertMonitor class function and will send the alert JSON string
           as parameter.
        ...
        Since actuator response and alerts comes on same channel we need to bifurcate them.
        """
        ...
        if status:
            ...
            Log.debug(f"Marking sensor response as acknowledged. status: {status}")
            self.comm_client.acknowledge()
    ...
    def _convert_to_csm_schema(self, message):
        ...
        try:
            ...
            if resource_type:
                ...
                2. enclosure, iem
                Hence splitting the above by colon(:) and assigning the last element from the split
                to module_type
                """
                ...
                input_alert_payload.convert(self.mapping_dict.get(const.COMMON), csm_alert_payload)
                ...
                for some alerts so we cannot include it in mapping file.
                """
                ...
                """
                Below mentioned fields are managed by CSM so they are not the part of mapping table
                """
                csm_schema[const.ALERT_NODE_ID] = \
                    csm_schema.get(const.ALERT_EXTENDED_INFO).get(const.ALERT_INFO)\
                ...
                csm_schema[const.ALERT_CREATED_TIME] = int(csm_schema[const.ALERT_CREATED_TIME])
                ...
                """
                1. Event Details field is of type List and will contain array of
                ...
                5. This string uniquely identifies the resource for which an alert has come.
                """
                ...
                if const.ALERT_EVENTS in csm_schema and csm_schema[const.ALERT_EVENTS] is not None:
            ...
            Log.error(f"Error occurred in coverting alert to csm schema. {e}")
        Log.debug(f"Converted schema:{csm_schema}")
        return csm_schema

    @staticmethod
    def _prepare_specific_info(csm_schema, specific_info):
        """
        ...
        if csm_schema[const.ALERT_MODULE_TYPE] in (const.ALERT_LOGICAL_VOLUME,
            ...
            for items in csm_schema[const.ALERT_EVENTS]:
                ...
                dictionaries containing name, health-reason and health-recommendation.
                2. For Sideplane expander we do not get name as a key instead we get component-id.
                """
    ...
    @staticmethod
    def _set_health_info(csm_schema, specific_info):
        try:
            if not isinstance(specific_info, list):
                csm_schema[const.ALERT_HEALTH] = specific_info.get(const.ALERT_HEALTH, "")
                csm_schema[const.DESCRIPTION] = specific_info.get(const.ALERT_HEALTH_REASON, "")
