...
class HealthPlugin(CsmPlugin):
    ...
    It has a callback which is called to send the received response to health service.
    Note, Health Plugin needs to be called in thread context as it blocks while listening for
    the response.
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            node_request_path = Conf.get(const.CSM_GLOBAL_INDEX, 'HEALTH.node_actuator_request')
    ...
    def _send_actuator_request_payload(self):
        ...
        try:
            ...
            Log.warn(f"Sending actuator request failed. Reason: {e}")
    ...
    def _send_encl_request(self, key, today):
        try:
            ...
            self.comm_client.send(self._storage_request_dict, is_storage_request=True)
            self._no_of_request += 1
            Log.debug(f"Sent storage enclosure request for {key}")
        except Exception as e:
            Log.warn(f"Sending enclosure request failed. Reason: {e}")

    def _send_node_request(self, key, today):
        try:
            self._node_request_dict[const.TIME] = str(today)
            self._node_request_dict[const.ALERT_MESSAGE][const.HEADER][const.UUID] = \
                str(uuid.uuid1())
            self._node_request_dict[const.ALERT_MESSAGE][const.ACT_REQ_TYPE][
                const.NODE_CONTROLLER][const.NODE_REQ] = const.NODE_HW + str(key)
            self.comm_client.send(self._node_request_dict, is_storage_request=False)
            ...
            Log.warn(f"Sending node request failed. Reason : {e}")

    def init(self, callback_fn, db_update_callback_fn):
        """
        Establish connection with the RMQ Server.

        :param callback_fn: This parameter specifies the name of HealthMonitor class function
            to which plugin will send the response as JSON string.
        :param db_update_callback_fn: This parameter specifies the name of HealthMonitor class
            function to update health map using db.
        """
    ...
    def health_plugin_callback(self, message):
        """
        This is the callback method on which we will receive the response from Comm class.

        :param message: Actual actuator response as  JSON string
        """
        ...
        if self.health_callback:
            try:
                ...
                Log.warn(f"SOme issue occurred in parsing and updating health: {e}")
        return status
    ...
    def update_health_map_with_alert(self, alert):
        ...
        try:
            ...
            if status:
                Log.debug(f"Health map update by alert successfull. status: {status}")
        except Exception as e:
            Log.warn(f"Health map update by alert failed. {e}")
    ...
    def _parse_alert(self, message):
        ...
        try:
            ...
            if resource_type.split(':')[0] == const.ENCLOSURE:
                mapping_key = self._health_mapping_dict.get(resource_type).get(const.KEY)
            else:
                mapping_key = self._health_mapping_dict.get(const.COMMON).get(const.KEY)
            ...
            resource_schema[const.HEALTH_ALERT_TYPE] = message.get(const.ALERT_STATE, "")
            resource_schema[const.ALERT_SEVERITY] = message.get(const.ALERT_SEVERITY, "")
            ...
            resource_schema[const.RESOURCE_KEY] = self._prepare_resource_key(resource_schema)
            health_value = self._derive_health(message, resource_schema[const.HEALTH_ALERT_TYPE])
            # This a special case, so handling separately.
            ...
            Log.warn(f"Parsing of health map by alert failed. {e}")
        return resource_schema

    @staticmethod
    def _derive_health(message, alert_type):
        """
        ...
        1. We have the health key but it has no value. i.e 'health': ''
        2. The health key itself is missing from the payload.
        ...
        try:
            ...
            if actuator_response:
                ...
                if resource_type:
                    ...
                    health_schema[const.RESOURCE_KEY] = self._prepare_resource_key(health_schema)
                    if "node" in resource_type.lower():
            ...
            Log.warn(f"Schema conversion failed:{e}")
        return health_schema

    @staticmethod
    def _parse_specific_info(health_schema, health_field, res_id_field):
        ...
        for items in health_schema.get(const.SPECIFIC_INFO, []):
            ...
            # There might be a situation where we have 'health' as the key but its value is empty,
            # i.e 'health': ''. In that scenario we will replace it with 'NA'
    ...
    @staticmethod
    def _prepare_resource_key(health_schema):
        ...
        try:
            ...
            Log.warn(f"Preparation of resource key failed: {e}")
        return resource_key

    def _send(self):
        """
        This is thread function.
        This method sends the actuator request to RMQ for receiving health of resources.
        """
    ...
    @staticmethod
    def _handle_sas_alert(resource_schema, health_value, extended_info):
        """
        ...
        try:
            ...
            Log.warn(f"Handling SAS alert for health update failed. {e}")
