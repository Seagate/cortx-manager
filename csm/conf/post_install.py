...
class PostInstall(Setup):
    ...
    def set_cluster_id(self):
        ...
        Conf.set(const.CSM_GLOBAL_INDEX, const.KEY_CLUSTERID,
                clusterid)
    ...
    def set_node_id(self):
        ...
        """
        node_ids = []
        Log.info("Fetching node ids for each storage set.")
        num_storage_sets = Conf.get(const.CONSUMER_INDEX, 'cluster>num_storage_set')
        for index in range(num_storage_sets):
            node_ids =  node_ids +  Conf.get(const.CONSUMER_INDEX, f'cluster>storage_set[{index}]>nodes')

        Log.info("Setting node ids.")
        for id in range(len(node_ids)):
            Conf.set(const.CSM_GLOBAL_INDEX, f"const.KEY_NODE_ID[{id}]",
                f"node_ids[{id}]")
