...
class Configure(Setup):
    ...
    def _configure_csm_ldap_schema(self):
        ...
        csm_schema_version = Conf.get(const.CSM_GLOBAL_INDEX,
                                    f"{const.OPENLDAP_KEY}>csm_schema_version")
        ...
        tmpl_init_data = tmpl_init_data.replace('<version>',f"dc={csm_schema_version}")
        ...
        tmpl_useracc_data = tmpl_useracc_data.replace('<version>',f"dc={csm_schema_version}")
