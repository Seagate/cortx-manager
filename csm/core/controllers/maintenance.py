...
from .validators import Enum, PortValidator, Server, ValidationErrorFormatter
from .view import CsmAuth, CsmView


class GetMaintenanceSchema(Schema):
    action = fields.Str(required=True,
                        validate=[Enum([const.NODE_STATUS, const.REPLACE_NODE_STATUS])])
...
class MaintenanceView(CsmView):
    ...
    async def post(self):
        ...
        try:
            PostMaintenanceSchema().load(body, unknown=const.MARSHMALLOW_EXCLUDE)
        except ValidationError as e:
            raise InvalidRequest(ValidationErrorFormatter.format(e))
