...
class AlertsHistoryQueryParameter(Schema):
    ...
    offset = fields.Int(validate=validate.Range(min=0), allow_none=True, default=0, missing=0)
    # Setting the limit to 1000 to limit the alerts records.
    page_limit = fields.Int(data_key='limit', default=1000, validate=validate.Range(min=0),
                            missing=1000)
    sort_by = fields.Str(data_key='sortby', default="created_time", missing="created_time")
    direction = fields.Str(data_key='dir', validate=validate.OneOf(['desc', 'asc']), missing='desc',
                           default='desc')
    sensor_info = fields.Str(default=None, missing=None, allow_none=True)
    start_date = fields.Str(data_key='start_date', default=None, missing=None, allow_none=True)
    end_date = fields.Str(data_key='end_date', default=None, missing=None, allow_none=True)

    @staticmethod
    @validates('duration')
    def validate_duration(value):
        if value:
            int(re.split(r'[a-z]', value)[0])
            time_format = re.split(r'[0-9]', value)[-1]
            dur = {"s": "seconds", "m": "minutes", "h": "hours", "d": "days"}
            if time_format not in dur.keys():
                raise InvalidRequest("Invalid Parameter for Duration",
                                     ALERTS_MSG_INVALID_DURATION)

    @staticmethod
    @validates_schema
    def check_date(data: Dict, *args, **kwargs):
        if data["start_date"]:
            ...
            if data["start_date"] > date.today() or data["end_date"] > date.today():
                raise ValidationError("Start/End date cannot be greater than today.")
...
class AlertsHistoryListView(CsmView):
    ...
    async def get(self):
        ...
        try:
            alerts_history_data = alerts_history_qp.load(self.request.rel_url.query,
                                                         unknown='EXCLUDE')
        except ValidationError as e:
            raise InvalidRequest(ValidationErrorFormatter.format(e))
