...
class AlertsView(CsmView):
    ...
    async def patch(self):
        ...
        try:
            ...
            raise InvalidRequest("Invalid Parameter for alerts", str(e))
...
class AlertCommentsCreateSchema(Schema):
    ...
    comment_text = fields.Str(
        required=True, validate=[validate.Length(min=1, max=const.STRING_MAX_VALUE)])
...
class AlertCommentsView(CsmView):
    ...
    async def post(self):
        ...
        return await self.alerts_service.add_comment_to_alert(alert_uuid, self.user_id,
