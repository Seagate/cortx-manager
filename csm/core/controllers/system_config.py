...
from .validators import Ipv4, Server
...
class Ipv4BaseSchema(Schema):
    """
    Ipv4 base schema class for common ipv4 setting in management network and data network settings.
    """

    is_dhcp = fields.Boolean(allow_none=True)
    nodes = fields.List(fields.Nested(Ipv4NodesSchema, allow_none=True, unknown='EXCLUDE'))
...
class ManagementNetworkSettingsSchema(Schema):
    ...
    ipv6 = fields.Nested(ManagementNetworkIpv6Schema, allow_none=True, unknown='EXCLUDE')
...
class DataNetworkSettingsIpv6Schema(Schema):
    ...
    nodes = fields.List(fields.Nested(Ipv6NodesSchema, allow_none=True, unknown='EXCLUDE'))
...
class DataNetworkSettingsSchema(Schema):
    ...
    ipv4 = fields.Nested(Ipv4BaseSchema, allow_none=True, unknown='EXCLUDE')
    ipv6 = fields.Nested(DataNetworkSettingsIpv6Schema, allow_none=True, unknown='EXCLUDE')
...
class DnsNetworkSettingsSchema(Schema):
    ...
    nodes = fields.List(fields.Nested(DnsNetworkSettingsNodes, allow_none=True, unknown='EXCLUDE'))
...
class DateTimeSettingsSchema(Schema):
    ...
    date_time = fields.Nested(ManualDateTimeSchema, allow_none=True, unknown='EXCLUDE')
...
class EmailConfigSchema(Schema):
    ...
    smtp_sender_password = fields.Str(validate=validate.Length(min=4, max=64), allow_none=True)
    email = fields.Str(allow_none=True)

    @staticmethod
    @validates('smtp_protocol')
    def validate_smtp_protocol(value):
...
class LdapConfigSchema(Schema):
    ...
    user_search_base = fields.Str(validate=validate.Length(min=4, max=32), allow_none=True)
...
class SystemConfigSettingsSchema(Schema):
    ...
    """
    appliance_name = fields.Str(
        validate=validate.Regexp('^[A-Za-z0-9_-]{2,255}$'), allow_none=True, unknown='EXCLUDE')
    management_network_settings = fields.Nested(
        ManagementNetworkSettingsSchema, allow_none=True, unknown='EXCLUDE')
    data_network_settings = fields.Nested(
        DataNetworkSettingsSchema, allow_none=True, unknown='EXCLUDE')
    dns_network_settings = fields.Nested(
        DnsNetworkSettingsSchema, allow_none=True, unknown='EXCLUDE')
    date_time_settings = fields.Nested(DateTimeSettingsSchema, allow_none=True, unknown='EXCLUDE')
    notifications = fields.Nested(NotificationSchema, allow_none=True, unknown='EXCLUDE')
...
class SystemConfigConverter:
    ...
    def _view_to_model(d: Dict) -> Dict:
        ...
        :return: Modified input dictionary representing a system config model
        """
    ...
    def _model_to_view(d: Dict) -> Dict:
        ...
        :return: Modified input dictionary representing a system config view
        """
        if (isinstance(d[const.APPLIANCE_NAME], dict)
                and d[const.APPLIANCE_NAME].get(const.APPLIANCE_NAME) is not None):
...
class SystemConfigListView(CsmView):
    ...
    async def post(self):
        ...
        try:
            ...
            config_data = schema.load(await self.request.json(), unknown='EXCLUDE')
...
class ConfigTypeSchema(Schema):
    ...
    config_type = fields.String(required=True, validate=validate.OneOf(const.SYSCONFIG_TYPE))
...
class SystemConfigView(CsmView):
    ...
    async def get(self):
        ...
        id_var = self.request.match_info["config_id"]
        system_config = await self._service.get_system_config_by_id(id_var)
    ...
    async def put(self):
        ...
        system_config = await self._service.update_system_config(
            id_var, SystemConfigView._converter._view_to_model(config_data))
        return SystemConfigView._converter._model_to_view(system_config)

    @CsmAuth.permissions({Resource.MAINTENANCE: {Action.UPDATE}})
    @CsmView.asyncio_shield
    async def patch(self):
        """PATCH REST implementation for updating a system config data based on config type"""
        ...
        try:
            ...
            config_data = system_config_schema.load(await self.request.json(), unknown='EXCLUDE')
            request_data = config_type_schema.load(self.request.rel_url.query, unknown='EXCLUDE')
        except json.decoder.JSONDecodeError:
            raise InvalidRequest(message_args="Request body missing")
        except ValidationError as e:
            raise InvalidRequest(f"Invalid request body: {e}")
        ...
        system_config = await self._service.update_system_config_by_type(
            config_type, id_var, SystemConfigView._converter._view_to_model(config_data))
        return SystemConfigView._converter._model_to_view(system_config)
...
class ProvisionerStatusSchema(Schema):
    ...
    status_type = fields.String(
        required=True, validate=validate.OneOf(const.PROVISIONER_CONFIG_TYPES))


@CsmView._app_routes.view("/api/v1/provisioner/status")
class ProvisionerStatus(CsmView):
    """
    Rest implementation to get provisioner config status like success or fail
    ...
    async def get(self):
        ...
        try:
            request_data = provisioner_status.load(self.request.rel_url.query, unknown='EXCLUDE')
        except ValidationError as e:
            raise InvalidRequest("Invalid parameter for provisioner status", str(e))
