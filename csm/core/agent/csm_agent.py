...
class CsmAgent:
    ...
    def init():
        ...
        if Conf.get(const.CSM_GLOBAL_INDEX, "DEPLOYMENT.mode") != const.DEV:
            Conf.decrypt_conf()
        from cortx.utils.data.db.db_provider import (  # pylint: disable=import-outside-toplevel
            DataBaseProvider, GeneralConfig)
        ...
        cached_files = glob.glob(f'{const.CSM_TMP_FILE_CACHE_DIR}/*')
        ...
        health_service = HealthAppService(health_repository, alerts_repository, health_plugin_obj)
        CsmAgent.health_monitor = HealthMonitorService(health_plugin_obj, health_service)
        ...
        CsmAgent.alert_monitor = AlertMonitorService(
            alerts_repository, pm.AlertPlugin(), CsmAgent.health_monitor.health_plugin,
            http_notifications)
        ...
        CsmRestApi._app.login_service = LoginService(auth_service, user_manager, role_manager,
                                                     session_manager)
        ...
        CsmRestApi._app[const.FW_UPDATE_SERVICE] = FirmwareUpdateService(
            provisioner, Conf.get(const.CSM_GLOBAL_INDEX, 'UPDATE.firmware_store_path'),
            update_repo)
        CsmRestApi._app[const.SYSTEM_CONFIG_SERVICE] = SystemConfigAppService(
            db, provisioner, security_service, system_config_mgr,
        ...
        CsmRestApi._app[const.MAINTENANCE_SERVICE] = MaintenanceAppService(
            CortxHAFramework(), provisioner, db)
...
if __name__ == '__main__':
    ...
    from csm.core.agent.api import AlertHttpNotifyService, CsmRestApi
    ...
    from csm.core.services.alerts import (AlertEmailNotifier, AlertMonitorService, AlertRepository,
                                          AlertsAppService)
    ...
    from csm.core.services.health import HealthAppService, HealthMonitorService, HealthRepository
    ...
    from csm.core.services.sessions import AuthService, LoginService, SessionManager
