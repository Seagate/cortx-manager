...
class S3AccountService(S3BaseService):
    ...
    async def create_account(self, account_name: str, account_email: str, password: str) -> Dict:
        ...
        Log.debug(f"Create S3 account completed in {end_time-start_time} ms")
    ...
    async def get_account(self, account_name) -> dict:
        ...
        if account is None:
            return None
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Get S3 account completed in {end_time-start_time} ms")
        return {
            "account_name": account.account_name,
            "account_email": account.account_email
        }
    ...
    async def list_accounts(self, session, continue_marker=None, page_limit=None,
        ...
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Get List of S3 accounts completed in {end_time-start_time} ms")
        return resp
    ...
    async def patch_account(self, s3_session: S3Credentials, account_name: str,
        ...
        if password:
            ...
            if isinstance(new_profile, IamError):
                ...
                self._handle_error(new_profile)
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Update S3 account completed in {end_time-start_time} ms")
        return response
    ...
    async def delete_account(self, s3_session: S3Credentials, account_name: str):
        ...
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Delete S3 account completed in {end_time-start_time} ms")
