...
class S3BucketService(S3BaseService):
    ...
    async def create_bucket(self, s3_session: S3Credentials,
        ...
        Log.debug(f"Create bucket completed in {end_time-start_time} ms")
        return {
    ...
    async def list_buckets(self, s3_session: S3Credentials) -> Dict:
        ...
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Get list of buckets completed in {end_time-start_time} ms")
    ...
    async def delete_bucket(self, bucket_name: str, s3_session: S3Credentials):
        ...
        try:
            ...
            self._handle_error(e)
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Delete bucket completed in {end_time-start_time} ms")
    ...
    async def get_bucket_policy(self, s3_session: S3Credentials,
        ...
        try:
            ...
            self._handle_error(e)
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Get bucket policy completed in {end_time-start_time} ms")
    ...
    async def put_bucket_policy(self, s3_session: S3Credentials, bucket_name: str,
        ...
        try:
            ...
            self._handle_error(e)
        end_time = int(round(time.time() * 1000))
        Log.debug(f"update bucket policy completed in {end_time-start_time} ms")
    ...
    async def delete_bucket_policy(self, s3_session: S3Credentials,
        ...
        try:
            ...
            self._handle_error(e)
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Delete bucket policy completed in {end_time-start_time} ms")
    ...
    async def get_bucket_tagging(self, s3_session: S3Credentials, bucket_name: str) -> Dict:
        ...
        try:
            ...
            self._handle_error(e)
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Get bucket tagging completed in {end_time-start_time} ms")
        return {"tagset": tags}
    ...
    async def put_bucket_tagging(self, s3_session: S3Credentials,
        ...
        try:
            ...
            self._handle_error(e)
        end_time = int(round(time.time() * 1000))
        Log.debug(f"Update bucket tagging completed in {end_time-start_time} ms")
