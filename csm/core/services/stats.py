...
class StatsAppService(ApplicationService):
    ...
    async def get(self, stats_id, panel, from_t, to_t, metric_list, interval, total_sample, unit,
                  output_format, query) -> Dict:
        ...
        panel_data = await self._stats_provider.process_request(
            stats_id=stats_id, panel=panel, from_t=from_t, duration_t=to_t, metric_list=metric_list,
            interval=interval, total_sample=total_sample, unit=unit.lower(),
            output_format=output_format, query=query)
    ...
    async def get_panels(self, stats_id, panels_list, from_t, to_t, interval, total_sample,
                         output_format) -> Dict:
        ...
        for panel in panels_list:
            panel_data = await self._stats_provider.process_request(
                stats_id=stats_id, panel=panel, from_t=from_t, duration_t=to_t, metric_list="",
                interval=interval, total_sample=total_sample, unit="",
                output_format=output_format, query="")
    ...
    async def get_metrics(self, stats_id, metrics_list, from_t, to_t, interval,
        ...
        Log.debug(f"Get metrics requested: id={stats_id}, interval={interval}")
        output = {}
        panels = {}
        try:
            ...
            raise CsmInternalError(f"Stats: Invalid metric list {metrics_list}")
        ...
        for panel in panels:
            panel_data = await self._stats_provider.process_request(
                stats_id=stats_id, panel=panel, from_t=from_t, duration_t=to_t,
                metric_list=panels[panel]["metric"], interval=interval, total_sample=total_sample,
                unit=panels[panel]["unit"], output_format=output_format, query="")
            data_list.extend(panel_data["list"])
        output["metrics"] = data_list
        Log.debug(f"Stats Request Output: {output}")
        return output
