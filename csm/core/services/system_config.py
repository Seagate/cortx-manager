...
from csm.common.email import EmailError, EmailSender
from csm.common.errors import CsmNotFoundError
from csm.common.queries import SortBy
from csm.common.services import ApplicationService
from csm.core.blogic import const
from csm.core.data.models.system_config import (ApplianceName, CertificateInstallationStatus,
...
class SystemConfigManager:
    ...
    async def create(self, system_config: SystemConfigSettings) -> SystemConfigSettings:
        """
        Stores a new system config

        :param system_config: System config settings model instance
        :return: System config settings object.
        """

        # TODO: Model Validation.
        return await self.storage(SystemConfigSettings).store(system_config)

    async def get_system_config_by_id(self, config_id: str) -> SystemConfigSettings:
        """
        Fetches system config based on id

        :param config_id: System config identifier
        :return: System config settings object in case of success. None otherwise.
        """

        query = Query().filter_by(Compare(SystemConfigSettings.config_id, '=', config_id))
        return next(iter(await self.storage(SystemConfigSettings).get(query)), None)
    ...
    async def get_system_config_list(self, offset: int = None, limit: int = None,
        ...
        :return: A list of System Config models
        """
        ...
        if sort:
            query = query.order_by(getattr(SystemConfigSettings, sort.field), sort.order)
    ...
    async def delete(self, config_id: str) -> None:
        ...
        await self.storage(SystemConfigSettings).delete(
            Compare(SystemConfigSettings.config_id, '=', config_id))
    ...
    async def create_license(self, license_arg: OnboardingLicense) -> OnboardingLicense:
        """
        Stores a new onboarding license key

        :param license_arg: Onboarding license model instance
        :return: Onboarding license object.
        """

        return await self.storage(OnboardingLicense).store(license_arg)
...
class SystemConfigAppService(ApplicationService):
    ...
    async def create_system_config(self, config_id: str, **kwargs) -> dict:
        ...
        :param config_id: system Config identifier
        :return: A dictionary describing the newly created system config.
        """
    ...
    async def get_system_config_list(self):
        ...
        :return: A list of System Config
        """

        system_config_list = await self.system_config_mgr.get_system_config_list()
        if not system_config_list:
            system_config_list = []
        return [system_config.to_primitive() for system_config in system_config_list]
    ...
    async def get_system_config_by_id(self, config_id: str):
        ...
        :param config_id: System config identifier
        :return: A dict of system config
        """

        system_config = await self.system_config_mgr.get_system_config_by_id(config_id)
        if not system_config:
            raise CsmNotFoundError("There is no such system config", SYSTEM_CONFIG_NOT_FOUND)
        return system_config.to_primitive()

    async def update_system_config(self, config_id: str, new_values: dict) -> dict:
        """
    ...
    async def test_email_config(self, config_data: dict) -> bool:
        ...
        target_emails = [x.strip() for x in config.email.split(',')]   # pylint: disable=no-member
        html_body = self.email_test_template.render()
        subject = const.CSM_SMTP_TEST_EMAIL_SUBJECT

        message = EmailSender.make_multipart(
            config.smtp_sender_email, config.email, subject, html_body)
        try:
            ...
            return {"status": False,
                    "error": "Some recipients did not receive the message",
                    "failed_recipients": success.keys()}
        except EmailError as e:
            return {"status": False, "error": str(e), "failed_recipients": target_emails}

    async def create_onboarding_license(self, csm_onboarding_license_key: str, **kwargs) -> dict:
        """
        Handles the onboarding license key store

        :param csm_onboarding_license_key: license key identifier
        :return: A dict describing the newly created onboarding license key.
        """
    ...
    async def get_provisioner_status(self, status_type):
        ...
        :return: Provisioner's success or failed status
        """
    ...
    async def update_system_config_by_type(self, config_type, config_id, config_data):
        ...
        if (config_data.get(const.MANAGEMENT_NETWORK)
                or config_data.get(const.DATA_NETWORK)
                or config_data.get(const.DNS_NETWORK)):
