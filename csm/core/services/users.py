...
from csm.common.errors import CsmNotFoundError, CsmPermissionDenied, InvalidRequest, ResourceExist
from csm.common.queries import SortBy
from csm.common.services import ApplicationService
from csm.core.blogic import const
from csm.core.data.models.users import Passwd, User
...
class UserManager:
    ...
    async def create(self, user: User) -> User:
        ...
        if existing_user:
            raise ResourceExist(
                f"User already exists: {existing_user.user_id}", USERS_MSG_ALREADY_EXISTS)
    ...
    async def get(self, user_id) -> User:
        ...
        :return: User object in case of success. None otherwise.
        """

        Log.debug(f"Get user service user id:{user_id}")
        # TODO: In absence of ComapareIgnoreCase manually filtering
        # query = \
        # Query().filter_by(Compare(User.to_native("user_id").lower(), '=', user_id.lower()))
...
class CsmUserService(ApplicationService):
    ...
    def _user_to_dict(user: User):
        """Helper method to convert user model into a dictionary representation"""
        return {
            "id": user.user_id,
            ...
            "roles": user.roles,
            "email": user.email,
            "created_time": f'{user.created_time.isoformat()}Z',
            "updated_time": f'{user.updated_time.isoformat()}Z'
        }
    ...
    async def create_user(self, user_id: str, password: str, **kwargs) -> dict:
        ...
        :return: A dictionary describing the newly created user.
    ...
    async def update_user(self, user_id: str, new_values: dict, loggedin_user_id: str) -> dict:
        ...
        if current_password and not self._verify_current_password(user, current_password):
    ...
    @staticmethod
    def _verify_current_password(user: User, password):
