...
from csm.common.queries import DateTimeRange, QueryLimits
...
class AuditLogManager():
    ...
    def _prepare_filters(self, component, create_time_range: DateTimeRange):
        range_condition = []
        range_condition = self._prepare_time_range(COMPONENT_MODEL_MAPPING[component]["field"],
                                                   create_time_range)
    ...
    async def retrieve_by_range(self, component, limits, time_range: DateTimeRange):
...
class AuditService(ApplicationService):
    ...
    @staticmethod
    def generate_audit_log_filename(component, start_time, end_time):
    ...
    @staticmethod
    def get_date_range_from_duration(start_date, end_date):
    ...
    async def create_audit_log_file(self, file_name, component, time_range):
        """create audit log file and compress to tar.gz"""
        try:
            ...
            await self.audit_mngr.count_by_range(component, time_range)
            ...
            for log in audit_logs:
                file.write(COMPONENT_MODEL_MAPPING[component]["format"].format(
                    **(log.to_primitive())) + "\n")
            file.close()
    ...
    async def get_by_range(self, component: str, start_time: str, end_time: str):
        ...
        if not COMPONENT_MODEL_MAPPING.get(component, None):
            raise CsmNotFoundError("No audit logs for %s" % component, COMPONENT_NOT_FOUND)

        time_range = self.get_date_range_from_duration(int(start_time), int(end_time))
        query_limit = QueryLimits(Conf.get(const.CSM_GLOBAL_INDEX, "Log.max_result_window"), 0)
        audit_logs = await self.audit_mngr.retrieve_by_range(component, query_limit, time_range)
        return [COMPONENT_MODEL_MAPPING[component]["format"].format(**(log.to_primitive()))
                for log in audit_logs]
