...
from smtplib import (SMTP, SMTP_SSL, SMTPAuthenticationError, SMTPException, SMTPHeloError,
...
class EmailSender:
    ...
    def _reconnect(self):
        for _ in range(1, self._config.reconnect_attempts + 1):
            try:
                ...
                raise ServerCommunicationError(e.smtp_error.decode(  # pylint: disable=no-member
                    'utf-8')) from None
    ...
    async def send_message(self, message: EmailMessage):
        ...
        :return: A dictionary that describes failed recipients
        :raise InvalidCredentialsError: The exception is raised when the provided login/password
               are not correct
        :raise OutOfAttemptsEmailError:
        :raise ServerCommunicationError: The exception is raised when server provides invalid
               responses
        :raise BadEmailMessageError: The exception is raised when it is attempted to send invalid
               message
        """
    ...
    def make_multipart(from_address=None, to_address=None, subject=None, html_text=None,
        ...
        :param to_address: In case of multiple recipients, join the emails by ", "
