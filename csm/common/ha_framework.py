    ...
    def init(self, force_flag):
        ...
        for ra in self._resource_agents.values():
            if not ra.init(force_flag):
                raise Exception(f'Error: initalizing resource agent {ra}')
        return True
...
class CortxHAFramework(HAFramework):
    def __init__(self, resource_agents=None):
        super().__init__(resource_agents)
    ...
    def get_nodes(self):
        ...
        if _rc not in [0, 1]:
            raise Exception(f"Failed: Command: {_live_node_cmd} Returncode: {_rc} Error: {_err}")
        if _output and _output.lower().strip() == "online":
            online = True
        return {"node_status": [{"name": "cluster", "online": online, "standby": not online}]}
    ...
    def make_node_active(node):
        ...
        try:
            ...
            return {"message": const.STATE_CHANGE.format(node="cluster", state='start')}
        except Exception as e:
            raise Exception(f"Failed to put {node} on active state. Error: {e}")
    ...
    def shutdown(self, node):
        ...
        shutdown_cron_time = Conf.get(const.CSM_GLOBAL_INDEX, "MAINTENANCE.shutdown_cron_time")
...
class PcsHAFramework(HAFramework):
    ...
    def get_nodes(self):
        ...
        _live_node_cmd = const.HCTL_NODE.format(
            command='status', user=self._user, pwd=self._password)
    ...
    def make_node_active(self, node):
        ...
        try:
            ...
            _standby_cmd = const.HCTL_NODE.format(
                command=_command, user=self._user, pwd=self._password)
    ...
    def make_node_passive(self, node):
        ...
        try:
            ...
            _unstandby_cmd = const.HCTL_NODE.format(
                command=_command, user=self._user, pwd=self._password)
            ...
            raise Exception(f"Failed to remove {node} from passive state. Error: {e}")
    ...
    def shutdown(self, node):
        ...
        _cluster_shutdown_cmd = _command.format(
            node=node, user=self._user, pwd=self._password, CSM_PATH=const.CSM_PATH)
        shutdown_cron_time = Conf.get(const.CSM_GLOBAL_INDEX, "MAINTENANCE.shutdown_cron_time")
...
class PcsResourceAgent(ResourceAgent):
    ...
    def is_available(self):
        ...
        for resource in self._resources:
            _chk_res_cmd = f"pcs resource show {resource}"
    ...
    def _delete_resource(self):
        for resource in self._resources:
            _delete_res_cmd = f"pcs resource delete {resource}"
            _proc = SimpleProcess(_delete_res_cmd)
            _output, _err, _rc = _proc.run(universal_newlines=True)
            if _err != '':
                raise Exception(
                    f"Failed: Command: {_delete_res_cmd} Error: {_err} Returncode: {_rc}")
    ...
    def _ra_init(self):
        ...
        if not os.path.exists("/var/tmp"):
            os.makedirs("/var/tmp")
        self._cmd_list.append(f"pcs cluster cib {self._resource_file}")

    def _init_resource(self, resource, service, provider, interval, timeout):
        _cmd = (f"pcs -f {self._resource_file} resource create {resource} {provider}:{service} "
                f"meta failure-timeout=10s op monitor timeout={timeout[1]} interval={interval[1]} "
                f"op start timeout={timeout[0]} interval={interval[0]} "
                f"op stop timeout={timeout[2]} interval={interval[2]}")
        self._cmd_list.append(_cmd)

    def _init_constraint(self, score):
        # Configure colocation
        self._cmd_list.append(
            f"pcs -f {self._resource_file} constraint colocation set {' '.join(self._resources)}")

        # Configure update
        self._cmd_list.append(
            f"pcs -f {self._resource_file} constraint order set {' '.join(self._resources)}")

        # Configure score
        for resource in self._resources:
            self._cmd_list.append(f"pcs -f {self._resource_file} "  # pylint: disable=no-member
                                  f"constraint location {resource} prefers {self._primary}={score}")
            self._cmd_list.append(f"pcs -f {self._resource_file} "  # pylint: disable=no-member
                                  f"constraint location {resource} "
                                  f"prefers {self._secondary}={score}")

    def _execute_config(self):
        self._cmd_list.append(f"pcs cluster cib-push {self._resource_file}")
        for cmd in self._cmd_list:
            _proc = SimpleProcess(cmd)
            _output, _err, _rc = _proc.run(universal_newlines=True)
            if _err != '':
                raise Exception(f"Failed: Command: {cmd} Error: {_err} Returncode: {_rc}")
