...
class TimeSeriesProvider:
    ...
    async def get_metrics(self):
        ...
        for panel in self._agg_rule.keys():
            for metric in self._agg_rule[panel]["metrics"]:
                metrics.append(f"{panel}.{metric.get('name')}")

        return metrics

    async def _validate_panel(self, panel):
        """Validate panel from aggregation rule"""
...
class TimelionProvider(TimeSeriesProvider):
    ...
    def __init__(self, agg_rule):
        ...
        self._url = f"{protocol}{host}:{port}/api/timelion/run"
        self._header = {'Content-Type': 'application/json',
                        'Accept': 'application/json, text/plain, */*',
                        'kbn-xsrf': 'anything',
                        'Connection': 'keep-alive'}
        self._timelion_req_body = Template(
            '{"sheet": [ "$query"], "time": '
            '{ "from": "$from_t", "interval": "$interval", "mode":"quick", "to":"$to_t" }}')
        self._timelion_query = Template(
            '.es(q=$metric, timefield=$timestamp, index=$index, metric=$method).$processing()')
        self._metric_set = None
        self._config_list = None
        self._aggr_rule = None
        self._indexes = None
        self._storage_interval = None
        self._offset_interval = None

    def init(self):
        try:
            super().init()
            self._storage_interval = int(Conf.get(
                const.CSM_GLOBAL_INDEX, 'STATS.PROVIDER.interval'))
            ...
            for panel in self._agg_rule.keys():
                ...
                for metric in self._agg_rule[panel]["metrics"]:
                    query = \
                        f'({self._parse(metric["node"], panel, output="")}).label({metric["name"]})'
        ...
        for node in nodes:
            if type(node["val"]) is str:
                if node["val"] in self._metric_set.keys():
                    ...
                    output = f'({output}).{self._metric_set[node["val"]]}'
    ...
    async def process_request(self, stats_id, panel, from_t, duration_t, metric_list, interval,
                              total_sample, unit, output_format, query):
        """
        Process request comming from csm stats api

        :param stats_id: Request id for aggregation
        :param panel: Which type metric throughput, iops, etc.
        :param from_t: Starting time of stats
        :param duration_t: Ending time of stats
        :param metric_list: List of labels
        :param interval: Difference between two datapoint [default: auto]
        :param output_format: Json format either redable or gui. [default: gui]
        :param query: Optional direct query to timelion_api
        """
        ...
        try:
            interval, duration_t, from_t = await self._parse_interval(from_t, duration_t, interval,
                                                                      total_sample)
            from_t = f"{datetime.utcfromtimestamp(int(from_t)).isoformat()}.000Z"
            duration_t = f"{datetime.utcfromtimestamp(int(duration_t)).isoformat()}.000Z"
            panel = panel.lower()
            metric_list, unit_list = await self._get_metric_list(panel, metric_list, unit)
            res = await self._aggregate_metric(panel, from_t, duration_t, interval, metric_list,
                                               query)
            return await self._convert_payload(res, stats_id, panel, output_format, unit_list)
        except Exception as e:
            Log.debug(f"Failed to request stats {e}")
            raise CsmInternalError(
                f"id: {stats_id}, Error: Failed to process timelion request {e}")
    ...
    async def get_all_units(self):
        ...
        for panel in self._agg_rule.keys():
            for metric in self._agg_rule[panel]["metrics"]:
                st = f"{panel}.{metric.get('name')}." + (await self.get_axis(panel))['y']
                if st not in mu:
                    mu.append(st)
                if panel == "throughput":
                    for sz in self._SIZE_DIV:
                        st = f"{panel}.{metric.get('name')}.{sz}"
                        if st not in mu:
                            mu.append(st)
        return mu
    ...
    async def _get_metric_list(self, panel, metric_list, unit):
        ...
        for i, metric in enumerate(metric_list):
            if metric not in aggr_panel:
                raise CsmInternalError(f"Invalid label {metric} for {panel}")
            if type(unit) is list:
    ...
    async def _parse_interval(self, from_t, duration_t, interval, total_sample):
        """Check from_t, duration_t time interval and calculate interval from total_sample"""
        ...
        if total_sample == "" and interval == "":
            ...
            interval = f"{int(diff_sec/int(total_sample))}s"
        elif interval != "":
    ...
    async def _update_index(self, metric, from_t, duration_t):
        ...
        for index in self._indexes:
            if index in metric:
                ...
                break
        if f_li == d_li:
            new_index = old_index.replace("*", '.'.join(list(f_li)))
    ...
    async def _aggregate_metric(self, panel, from_t, duration_t, interval, metric_list, query):
        ...
        if query == "":
            ...
            for metric in metric_list:
                query = query + await self._update_index(aggr_panel[metric], from_t, duration_t) \
                    + ','
            query = query[:-1] + ')'
        body = self._timelion_req_body.substitute(query=query, from_t=from_t, interval=interval,
                                                  to_t=duration_t)
    ...
    async def _query(self, data):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self._url, json=data, headers=self._header) as resp:
