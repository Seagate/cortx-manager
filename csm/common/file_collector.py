...
class FileCollector:
    ...
    def collect(self, comp_name_list):
        ...
        try:
            self._startup()  # pylint: disable=no-member
            for comp_name in comp_name_list:
                ...
                self._execute_commands(comp_name, out_dir)  # pylint: disable=no-member
                ...
                for file_spec in self._collection_rules[comp_name]['files']:
                    ...
                    file_list.extend(self._expand_file_spec(file_spec))  # pylint: disable=no-member

                self._collect_files(file_list, out_dir)  # pylint: disable=no-member

            self._cleanup()  # pylint: disable=no-member
...
class LocalFileCollector(FileCollector):
    ...
    @staticmethod
    def _expand_file_spec(file_spec):
        Log.debug('file_spec: %s' % file_spec)
        cmd = 'ls %s' % file_spec
        output = subprocess.check_output(cmd, stderr=subprocess.PIPE, shell=True)
        return output.split('\n')
    ...
    @staticmethod
    def _collect_files(file_list, out_dir):
...
class RemoteFileCollector(FileCollector):
    ...
    def __init__(self, collection_rules, node, user, target_path):
        super().__init__(collection_rules, target_path)
    ...
    def _collect_files(self, file_list, out_dir):
        ...
        files = ' '.join(file_list)
        tar_cmd = f'tar czvf {remote_file} {files}'

        try:
            Log.debug(f'cmd: {tar_cmd}')
            self._channel.execute(tar_cmd)
            Log.debug(f'Copy remote:{remote_file} local:{local_file}')
            self._channel.recv_file(remote_file, local_file)
            self._channel.execute(f'rm -f {remote_file}')
        except OSError as e:
            Log.exception(e)
            raise CsmError(e.errno, str(e))
