...
class Payload:
    ...
    def convert(self, map_par, payload):
        """
        Converts 1 Schema to 2nd Schema depending on mapping dictionary.

        :param map_par: mapping dictionary :type: Dict
        :param payload: Payload Class Object with desired Source.
        :return: :type: Dict
        Mapping file example - key <input schema> : value <output schema>
        """

        for key in map_par.keys():
            val = self.get(key)
            payload.set(map_par[key], val)
        return payload
...
class CommonPayload:
    ...
    def get_doc_type(self):
        ...
        try:
            ...
            raise KeyError(f"Unsupported file type: {e}")
        except Exception as e:
