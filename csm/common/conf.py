...
class Conf:
    ...
    def get(index, key, default_val=None):
        ...
        return Conf._payloads[index].get(key) if default_val is None else default_val
    ...
    @staticmethod
    def save(index=None):
        indexes = list(Conf._payloads) if index is None else [index]
    ...
    def decrypt_conf():
        ...
        for each_key in const.DECRYPTION_KEYS:
            cipher_key = Cipher.generate_key(cluster_id, const.DECRYPTION_KEYS[each_key])
            ...
            try:
                ...
                raise CipherInvalidToken(f"Decryption for {each_key} Failed. {e}")
