...
CSM_OPERATION_SUCCESSFUL = 0x0000
...
class CsmError(BaseError):
    ...
    def __init__(self, rc=0, desc=None, message_id=None, message_args=None):
        super().__init__(rc=rc, desc=desc, message_id=message_id, message_args=message_args)
...
class CsmSetupError(CsmError):
    ...
    def __init__(self, _desc=None):
        super().__init__(CSM_SETUP_ERROR, _desc)


class CommandTerminated(KeyboardInterrupt):
    """This error will be raised when some command is terminated during the processing"""
    _err = CSM_ERR_INTERRUPTED
    _desc = "Command is cancelled"

    def __init__(self, _desc=None):
        super().__init__(self._err, _desc)


class InvalidRequest(CsmError):
    """
    This error will be raised when an invalid response message is received for any of
    the cli commands.
    """
    ...
    def __init__(self, _desc=None, message_id=None, message_args=None):
        super().__init__(CSM_INVALID_REQUEST, _desc, message_id, message_args)
...
class ResourceExist(CsmError):
    ...
    def __init__(self, _desc=None, message_id=None, message_args=None):
        super().__init__(CSM_RESOURCE_EXIST, _desc, message_id, message_args)
...
class CsmInternalError(CsmError):
    ...
    def __init__(self, desc=None, message_id=None, message_args=None):
        super().__init__(CSM_INTERNAL_ERROR, f'Internal error: {desc}', message_id, message_args)


class CsmNotFoundError(CsmError):
    """This error is raised for all cases when an entity was not found"""
    def __init__(self, desc=None, message_id=None, message_args=None):
        super().__init__(CSM_INTERNAL_ERROR, desc, message_id, message_args)
