...
permissions = {"alerts": {"list": True, "update": True, "delete": True}}
...
def send_alerts(args):
    ...
    client.init()
    global expected_count  # pylint: disable=global-statement
    with open(f"{file_path}alert_input.json", 'r') as json_file:
        dict_var = json.load(json_file)
    # Creating CSM Schema Output for Comparison
    with open('alert_output_expected.json', 'w+') as out_file:
        out_file.write('[\n')
        out_file.close()
    count = 0
    for data in dict_var:
        count = count + 1
        with open(f"{file_path}alert_output_expected.json", 'a+') as out_file:
...
def recv_alerts_test2(args):
    with open(f"{file_path}alert_output.json", 'w+') as json_file:
        json_file.write('[\n')
        json_file.close()
...
def consume_alert_test1(message):
    global actual_count  # pylint: disable=global-statement
    actual_count = actual_count + 1
    return True


def consume_alert_test2(message):
    global count_test2  # pylint: disable=global-statement
    global expected_count  # pylint: disable=global-statement
    count_test2 = count_test2 + 1
    message.pop("updated_time")
    with open(f"{file_path}alert_output.json", 'a+') as json_file:
...
def test1(args):
    ...
    if actual_count != expected_count:
        raise TestFailed('Alerts count do not match. Actual : %d, Excepted :%d'
...
def compare_results():
    if not filecmp.cmp(f"{file_path}alert_output_expected.json", f"{file_path}alert_output.json"):
