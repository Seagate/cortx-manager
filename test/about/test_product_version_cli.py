...
permissions = {
            'product_version': { 'read': True }
}

show_csm_command = CommandFactory.get_command(
    ["product_version", "show"], permissions, Const.CLI_SCHEMA_PATH, [], [])

t = unittest.TestCase()
file_path = Const.MOCK_PATH

obtained_output_file = os.path.join("/tmp","product_version_table_obtained_output")
with open(file_path + "about/product_version_cmd_expceted_output_table") as fp:
    EXPECTED_OUTPUT = fp.readlines()

with open(file_path + "about/product_version_api_response") as fp:
    API_RESPONSE =  json.loads(fp.read())


with open(file_path + "about/product_version_commands_output.json") as fp:
    COMMANDS_OUTPUT = json.loads(fp.read())


def test_command_name(*args):
    expected_output = 'product_version'
    actual_output = show_csm_command.name
    t.assertEqual(actual_output, expected_output)


def test_command_options(*args):
    expected_output = COMMANDS_OUTPUT.get("expected_product_version_show")
