...
from csm.core.controllers.alerts.alerts import AlertCommentsView, AlertsListView, AlertsView
from csm.core.controllers.alerts.alerts_history import AlertsHistoryListView, AlertsHistoryView
from csm.core.controllers.audit_log import AuditLogDownloadView, AuditLogShowView
from csm.core.controllers.firmware_update import (FirmwarePackageAvailability,
                                                  FirmwarePackageUploadView,
                                                  FirmwareUpdateStatusView, FirmwareUpdateView)
from csm.core.controllers.health import HealthSummaryView, HealthView, NodeHealthView
...
from csm.core.controllers.s3.buckets import S3BucketListView, S3BucketPolicyView, S3BucketView
...
from csm.core.controllers.security import (SecurityInstallView, SecurityStatusView,
                                           SecurityUploadView)
from csm.core.controllers.stats import StatsPanelListView, StatsView
from csm.core.controllers.storage_capacity import StorageCapacityView
from csm.core.controllers.system_config import (OnboardingLicenseView, SystemConfigListView,
                                                SystemConfigView, TestEmailView)
from csm.core.controllers.users import AdminUserView, CsmUsersListView, CsmUsersView
from csm.core.services.permissions import PermissionSet
from csm.core.services.roles import RoleManager
from csm.test.common import (Const, TestFailed, assert_equal, assert_raises, async_return,
                             async_test, get_type_name)
...
async def test_manage_roles_with_root():
    role_manager = RoleManager(roles_dict)
    expected_permissions = PermissionSet({
        'alerts': {'delete', 'list', 'update'},
        'stats': {'delete', 'list', 'update'},
...
async def check_rest_ep_permissions(role_manager, handler, method, roles, must_fail):
    ...
    if roles:
        request.session.permissions = await role_manager.calc_effective_permissions(*roles)
...
async def test_rest_ep_permissions():
    """Check an accessibility of all REST API endpoints using predefined roles"""
    ...
    rest_ep_cases = [
        ...
        (FirmwarePackageAvailability, Method.GET, User.Anon, Access.FAIL),
        (FirmwarePackageAvailability, Method.GET, User.CsmAdmin, Access.OK),
        (FirmwarePackageAvailability, Method.GET, User.CsmUser, Access.FAIL),
        (FirmwarePackageAvailability, Method.GET, User.S3Account, Access.FAIL),
    ...
    for handler, method, roles, access in rest_ep_cases:
        await check_rest_ep_permissions(role_manager, handler, method, roles, not access)
