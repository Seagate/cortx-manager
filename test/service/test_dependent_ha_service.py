...
from provisioner import get_node_id
from statsd import StatsClient

sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))
from csm.common.comm import SSHChannel  # noqa: E402
from csm.common.conf import Conf  # noqa: E402
from csm.core.blogic import const  # noqa: E402
from csm.test.common import Const, TestFailed  # noqa: E402
...
def test_statsd():
    ...
    try:
        ...
        raise TestFailed(f"statsd service is not running. Error: {traceback.format_exc()}")
...
def test_kibana():
    ...
    try:
        ...
        cmds = ["systemctl is-active kibana.service", f"curl {url}"]
        results = ssh_execute(list(get_nodes_id().keys()), cmds)
        if 0 in results.values():
            for result in results:
                if results[result] != 0:
                    return
        raise TestFailed(f"{results.keys()} instace of kibana running on system")
    except Exception as e:
        raise TestFailed(f"Kibana is not working. Exception {e} ...")
...
def test_elasticsearch():
    ...
    try:
        ...
        cmds = ["systemctl is-active elasticsearch.service", f"curl {url}"]
        results = ssh_execute(list(get_nodes_id().keys()), cmds)
        for result in results.values():
            if result != 0:
                raise TestFailed(results)
    except Exception as e:
        raise TestFailed(f"Elasticsearch is not working. Exception {e} ...")
...
def test_rabbitmq():
    ...
    try:
        ...
        raise TestFailed(f"RabbitMQ is not working {e} ...")
